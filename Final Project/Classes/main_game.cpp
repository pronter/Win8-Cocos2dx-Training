...
void MainGame::is_crash(float tm)
{
	...
	for (int  i = enemy_list.size() - 1; i >= 0 ; --i)
	{
		...
		for (int i = bullet_list.size() - 1; i >= 0; --i)
		{
			...
			Size bulletLength = bullet->getContentSize();
			Size bulletLeftPoint = (Size)bulletPosition - Size(bulletLength.width / 4, 0);
			Size bulletRightPoint = (Size)bulletPosition + Size(bulletLength.width / 4, 0);
			if (enemy_size.containsPoint(bullet->getPosition())|| enemy_size.containsPoint(bulletLeftPoint)|| enemy_size.containsPoint(bulletRightPoint))
			{
